...
local_files=yutyrannus-chairlift
...
install -Dm755 "${local_files}/yutyrannus-brew-managed" "${root}/usr/libexec/yutyrannus-brew-managed"
...
printf '#!/bin/sh\nexec /usr/libexec/yutyrannus-brew-managed preinstall "$@"\n' > "${root}/usr/bin/brew-preinstall"
...
for operation in update upgrade; do
    ...
    printf '[Service]\nExecStart=\nExecStart=/usr/libexec/yutyrannus-brew-managed %s\n' "$operation" \
        > "${root}/usr/lib/systemd/system/brew-${operation}.service.d/yutyrannus-managed.conf"
done
